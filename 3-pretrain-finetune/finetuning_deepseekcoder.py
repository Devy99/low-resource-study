"""
Official implementation of the finetuning script for the DeepSeek Coder model from:
...
def _tokenize_fn(strings: Sequence[str], tokenizer: transformers.PreTrainedTokenizer) -> Dict:
    ...
    # Check how many strings overcome the maximum length
    count = 0
    for string in strings:
        if len(tokenizer(string).input_ids) > tokenizer.model_max_length:
            count += 1
    print(f"Number of strings that overcome the maximum length: {count}")
...
def train():
    ...
    print(f"Process data from {data_args.data_path}")
